...
function MemoryDecayChart({ subjects, topics, items, projection, onSelect, onCreate, onDelete, onReview }: MemoryDecayChartProps) {
  ...
  return (
    <section className="card chart-card cogn-span-full">
      ...
      {subjects.length > 0 && <form className="memory-add" onSubmit={submit}>
        ...
        <button className="button secondary" disabled={!concept.trim()} title="Cadastrar item na fila de repetição espaçada">Cadastrar item</button>
      </form>}
      ...
      {projection && <div className="memory-stats">
        ...
        <button className="text-button" onClick={() => onDelete(projection.itemId)}>Remover item</button>
      </div>}
      ...
      <div className="chart" style={{ height: 250 }}>
        ...
        </ResponsiveContainer> : <p className="cogn-empty">Cadastre um item de memória acima para projetar a curva: sem revisão a memória cruza 90% no Dia {projection?.optimalDay ?? "S"}, com a rotina Anki ela é rearmada toda vez que esquece.</p>}
      </div>
...
export function CognitiveCharts({ subjects, topics, schedule, executed, points, focusPoints, memoryItems, memoryProjection, onMemorySelect, onMemoryCreate, onMemoryDelete, onMemoryReview }: { subjects: Subject[]; topics: Topic[]; schedule: ScheduleResponse | null; executed: Map<number, number>; points: SubjectAccuracyPoint[]; focusPoints: FocusOverloadPoint[]; memoryItems: SpacedRepetitionItem[]; memoryProjection: MemoryDecayProjection | null; onMemorySelect: (id: number) => void; onMemoryCreate: (input: MemoryItemInput) => void; onMemoryDelete: (id: number) => void; onMemoryReview: (id: number, grade: ReviewGrade) => void }) {
  return (
    <div className="cognitive-section">
      ...
      <div className="cognitive-grid">
        ...
        <MemoryDecayChart subjects={subjects} topics={topics} items={memoryItems} projection={memoryProjection} onSelect={onMemorySelect} onCreate={onMemoryCreate} onDelete={onMemoryDelete} onReview={onMemoryReview} />
      </div>
    </div>
  );
}