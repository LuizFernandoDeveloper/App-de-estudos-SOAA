...
export function AdvancementView({ subjects, topics, materials, dashboard, demo, onChanged }: { subjects: Subject[]; topics: Topic[]; materials: StudyMaterial[]; dashboard: DashboardData | null; demo: boolean; onChanged: () => Promise<void> }) {
  ...
  return <div className="page-stack">
    ...
    {subjects.map((subject) => <SubjectCard key={subject.id} subject={subject} topics={topics.filter((topic) => topic.subjectId === subject.id)} materials={materials.filter((material) => material.subjectId === subject.id)} accuracy={accuracyById.get(subject.id) ?? null} demo={demo} onChanged={onChanged} />)}
...
function SubjectCard({ subject, topics, materials, accuracy, demo, onChanged }: { subject: Subject; topics: Topic[]; materials: StudyMaterial[]; accuracy: number | null; demo: boolean; onChanged: () => Promise<void> }) {
  ...
  const saveGoals = async () => {
    ...
    setSavingGoals(true);
    try {
      await api.updateSubjectGoals(subject.id, { goalAccuracy: goalDraft.accuracy, goalCoverage: goalDraft.coverage });
      await onChanged();
    } catch (error) {
  ...
  const cycleTopic = async (topic: Topic) => {
    if (demo) return;
    try {
      await api.updateTopic(topic.id, { subjectId: topic.subjectId, name: topic.name, status: nextStatus(topic.status) });
      await onChanged();
    } catch (error) {
      window.alert(String(error));
    }
  ...
  const deleteTopic = async (topic: Topic) => {
    if (demo || !window.confirm(`Remover o tópico "${topic.name}"?`)) return;
    try {
  ...
  const addTopic = async (event: FormEvent) => {
    ...
    if (!name) return;
    try {
      await api.createTopic({ subjectId: subject.id, name, status: "pendente" });
      setNewTopic("");
      await onChanged();
    } catch (error) {
      window.alert(String(error));
  ...
  const stepPage = async (material: StudyMaterial, delta: number) => {
    ...
    try {
      await api.updateMaterialPage(material.id, next);
      await onChanged();
    } catch (error) {
      window.alert(String(error));
    }
  ...
  return <section className="card subject-advance">
    ...
    <div className="advance-goals">
      ...
      <button className="button secondary" onClick={() => void saveGoals()} disabled={demo || savingGoals} title={demo ? "Desativado no modo demonstração" : undefined}><Save size={15} /> Salvar metas</button>
    </div>
    ...
    <div className="advance-topics">
      <div className="advance-block-title"><span>Tópicos</span>{!demo && <form className="topic-add" onSubmit={addTopic}><input value={newTopic} onChange={(event) => setNewTopic(event.target.value)} placeholder="Novo tópico (ex.: Logaritmos)" /><button className="button secondary" type="submit" title="Adicionar tópico"><Plus size={15} /> Adicionar</button></form>}</div>
      <div className="topic-chips">
        {topics.map((topic) => <span key={topic.id} className={`topic-chip ${topic.status}`} onClick={() => void cycleTopic(topic)} title={demo ? undefined : "Clique para avançar o status: pendente → em andamento → concluído"}>{topic.status === "concluido" && <CheckCircle2 size={13} />}{topic.name}<span className="topic-chip-status">{STATUS_LABEL[topic.status]}</span>{!demo && <button type="button" className="icon-button danger" title="Remover tópico" onClick={(event) => { event.stopPropagation(); void deleteTopic(topic); }}><X size={13} /></button>}</span>)}
    ...
    {materials.length ? <div className="advance-materials">
      ...
      {materials.map((material) => <div className="material-page-row" key={material.id}><BookOpen size={16} /><div className="material-page-info"><strong>{material.title}</strong><span>{material.category}</span></div><div className="material-page-track"><div className="progress-inline"><span style={{ width: `${material.totalPages ? Math.min(100, material.currentPage / material.totalPages * 100) : 0}%`, background: colorOf(subject.color) }} /></div><small>{material.currentPage} / {material.totalPages} pág</small></div>{!demo && <div className="page-stepper"><button className="icon-button" title="Voltar 5 páginas" onClick={() => void stepPage(material, -5)}><Minus size={15} /></button><button className="icon-button" title="Avançar 5 páginas" onClick={() => void stepPage(material, 5)}><Plus size={15} /></button></div>}</div>)}
